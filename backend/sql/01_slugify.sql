...
DECLARE found_equiv,
current_char VARCHAR(5);

DECLARE counter,
string_length,
temp_table_rows INT(10);
...
SET
  dirty_string = REPLACE(TRIM(REPLACE(dirty_string, '.', '-')), ' ', '-');
...
SET
  string_length = CHAR_LENGTH(dirty_string);

SET
  counter = 1;

-- Crete a temporary table that will hold the characters replacement map
CREATE TEMPORARY TABLE IF NOT EXISTS slug_temp_table (
...
SET
  is_allowed = LOCATE(current_char, allowed_chars);

IF is_allowed > 0 THEN
SET
  new_string = CONCAT(new_string, LOWER(current_char));
